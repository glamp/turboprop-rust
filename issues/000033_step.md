...
I will implement comprehensive testing for the MCP server following TurboProp's established two-tier testing methodology:

### Phase 1: Fast Unit Tests (< 10 seconds)
1. **Protocol Tests** - Test JSON-RPC message validation, serialization/deserialization
2. **Tool Tests** - Test SearchTool parameter validation and schema generation
3. **Server Tests** - Test MCP server creation, request handling without actual indexing
4. **Index Manager Tests** - Test manager creation, stats tracking, lifecycle without file watching
5. **Transport Tests** - Test transport creation and error response handling

### Phase 2: Integration Tests (30+ seconds)
1. **Binary Tests** - Spawn actual `tp mcp` processes and test CLI integration
2. **Library Tests** - Test end-to-end library usage with real file indexing
3. **Concurrent Tests** - Test server under concurrent load
4. **Search Integration** - Test actual search functionality with indexed repositories

### Phase 3: Test Infrastructure
1. **Common Utilities** - Reusable test fixtures, repository creation, assertion helpers
2. **Test Configuration** - Update test_config.toml with MCP-specific settings
3. **Test Data** - Create realistic test repositories with authentication, JWT, error handling code

### Implementation Strategy:
- Use Test Driven Development approach
- Mock expensive operations (ML model loading, file watching) in unit tests
- Use real operations in integration tests for full validation  
- Follow existing TurboProp test patterns and conventions
- Ensure tests cover both success and error scenarios comprehensively
- Maintain separation between fast feedback tests and comprehensive validation tests

This approach ensures robust testing coverage while maintaining the fast feedback loop essential for daily development.