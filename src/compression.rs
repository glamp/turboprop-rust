...
/// High-performance vector compressor
...
impl VectorCompressor {
    ...
    pub fn compress_batch(&mut self, vectors: &[Vec<f32>]) -> Result<Vec<CompressedVector>> {
        ...
        info!("Compressing {} vectors using {:?}", vectors.len(), self.config.algorithm);
        ...
        let compressed = match self.config.algorithm {
            ...
            CompressionAlgorithm::ProductQuantization => self.compress_product_quantization(vectors)?,
        ...
        info!("Compression completed: {:.1}% size reduction, {:.2}ms total",
    ...
    fn compress_scalar_quantization(&mut self, vectors: &[Vec<f32>]) -> Result<Vec<CompressedVector>> {
        ...
        vectors.iter().map(|vector| {
            let quantized_data = vector.iter()
                .map(|&value| {
                    let normalized = (value - global_min) / scale;
                    let quantized = normalized.round().min(num_levels as f32).max(0.0) as u8;
                    quantized
                })
                .collect();
    ...
    fn compress_product_quantization(&mut self, vectors: &[Vec<f32>]) -> Result<Vec<CompressedVector>> {
        if vectors.is_empty() {
            return Ok(Vec::new());
        }
        ...
        let num_subvectors = (vector_dim + subvector_size - 1) / subvector_size;
        let codebook_size = 256;  // 256 centroids per subspace

        info!("Building product quantization codebook: {} subvectors of size {}",
              num_subvectors, subvector_size);

        // Build codebook for each subspace
        let codebook = self.build_product_quantization_codebook(vectors, subvector_size, codebook_size)?;

        // Quantize vectors using the codebook
        let compressed = vectors.iter().map(|vector| {
            ...
            for i in 0..num_subvectors {
                ...
                let best_code = self.find_closest_centroid(subvector, &codebook[i]);
    ...
    fn decompress_single(&self, compressed: &CompressedVector) -> Result<Vec<f32>> {
        match compressed.metadata.algorithm {
            ...
            CompressionAlgorithm::ScalarQuantization => self.decompress_scalar_quantization(compressed),
            CompressionAlgorithm::ProductQuantization => self.decompress_product_quantization(compressed),
    ...
    fn decompress_scalar_quantization(&self, compressed: &CompressedVector) -> Result<Vec<f32>> {
        ...
        let decompressed = compressed.data.iter()
            .map(|&quantized| {
                params.min_value + (quantized as f32 * params.scale)
            })
            .collect();
    ...
    fn decompress_product_quantization(&self, compressed: &CompressedVector) -> Result<Vec<f32>> {
        let codebook = compressed.metadata.codebook.as_ref()
        ...
        for (subvector_idx, &code) in compressed.data.iter().enumerate() {
            if let Some(subspace_codebook) = codebook.get(subvector_idx) {
                if let Some(centroid) = subspace_codebook.get(code as usize) {
                    ...
                    return Err(anyhow::anyhow!("Invalid code {} for subvector {}", code, subvector_idx));
                }
            } else {
                return Err(anyhow::anyhow!("Missing codebook for subvector {}", subvector_idx));
            }
        }
    ...
    fn build_product_quantization_codebook(
        ...
        for i in 0..num_subvectors {
            ...
            let centroids = self.kmeans_centroids(&subvectors, codebook_size.min(subvectors.len()))?;
    ...
    fn update_stats(&mut self, original: &[Vec<f32>], compressed: &[CompressedVector], elapsed: std::time::Duration) {
        let original_bytes: usize = original.iter()
            .map(|v| v.len() * std::mem::size_of::<f32>())
            .sum();
        
        let compressed_bytes: usize = compressed.iter()
        ...
        self.stats.avg_compression_time_us = 
            elapsed.as_micros() as f64 / original.len() as f64;
    }
