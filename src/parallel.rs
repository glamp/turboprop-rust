...
impl ParallelFileProcessor {
    ...
    pub fn discover_files_parallel(&self, path: &Path, turboprop_config: &TurboPropConfig) -> Result<Vec<FileMetadata>> {
    ...
    pub async fn process_files_parallel(
        ...
        let chunk_results: Vec<Result<Vec<ContentChunk>, _>> = files
            ...
            .flat_map(|file_batch| {
                file_batch.par_iter().map(|file| {
                    self.process_single_file_chunks(file)
                })
            })
            .collect();
        ...
        for (idx, result) in chunk_results.into_iter().enumerate() {
            match result {
                ...
                Err(e) => {
                    let file_path = files.get(idx).map(|f| f.path.clone())
        ...
        info!("Generated {} chunks from {} files", all_chunks.len(), files_processed);

        // Generate embeddings in optimized batches
        let indexed_chunks = self.generate_embeddings_batch(&all_chunks, embedding_generator).await?;
        ...
        info!("Parallel processing completed: {} indexed chunks, {} failures, {:.2}ms avg per file",
              indexed_chunks.len(), errors.len(), stats.avg_file_processing_time_ms);
    ...
    fn process_single_file_chunks(&self, file: &FileMetadata) -> Result<Vec<ContentChunk>> {
        ...
        debug!("Generated {} chunks for {}", chunks.len(), file.path.display());
        Ok(chunks)
    }
    ...
    async fn generate_embeddings_batch(
        ...
        info!("Generating embeddings for {} chunks in batches of {}", 
              chunks.len(), self.config.embedding_batch_size);
        ...
        for chunk_batch in chunks.chunks(self.config.embedding_batch_size) {
            ...
            debug!("Generated embeddings for batch of {} chunks", chunk_batch.len());
        }

        info!("Completed embedding generation: {} indexed chunks", indexed_chunks.len());
        Ok(indexed_chunks)
    }
}
...
impl ParallelSearchProcessor {
    ...
    pub fn search_parallel(
        ...
        info!("Performing parallel search across {} chunks", indexed_chunks.len());
        
        // Calculate similarities in parallel
        let mut results: Vec<(f32, &IndexedChunk)> = indexed_chunks
            .par_iter()
            .map(|indexed_chunk| {
            ...
            .filter(|(similarity, _)| {
                threshold.map_or(true, |t| *similarity >= t)
            })
            .collect();
...
mod tests {
    ...
    fn test_parallel_file_discovery() {
        ...
        let files = processor.discover_files_parallel(temp_dir.path(), &turboprop_config).unwrap();
        assert!(files.len() >= 2);
    }
