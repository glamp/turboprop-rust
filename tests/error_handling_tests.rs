...
fn test_chunking_binary_file_rejection() {
    ...
    match result {
        Ok(chunks) => {
            ...
            assert!(
                chunks.is_empty() || chunks.iter().all(|c| c.content.len() < 50),
...
fn test_chunking_extremely_large_line() {
    ...
    assert!(
        result.is_ok(),
        "Large line file should be processed without errors"
    );
    let chunks = result.unwrap();
    ...
    assert!(
        !chunks.is_empty(),
        "Large line file should produce some chunks"
    );
    ...
    for chunk in chunks {
        assert!(
            chunk.content.len() < 3_000,
            "Individual chunks should be reasonably sized, got: {} chars",
            chunk.content.len()
        );
    }
}
...
fn test_file_discovery_with_invalid_unicode_filename() {
    ...
    #[cfg(unix)]
    {
        ...
        let invalid_filename =
            OsString::from_vec(vec![b'i', b'n', b'v', b'a', b'l', b'i', b'd', 0xFF, 0xFE]);
        ...
        if std::fs::write(&invalid_file_path, "some content").is_ok() {
            ...
            assert!(
                result.is_ok(),
                "File discovery should handle invalid unicode filenames"
            );
        }
    }
}
...
fn test_file_discovery_permission_denied_directory() {
    ...
    match result {
        Ok(files) => {
            ...
            assert!(
                files.is_empty(),
                "Should not find files in restricted directory"
            );
        }
        Err(_) => {
...
fn test_chunking_permission_denied_file() {
    ...
    let mut perms = std::fs::metadata(&restricted_file_path)
        .unwrap()
        .permissions();
    perms.set_mode(0o000); // No permissions
    ...
    assert!(
        result.is_err(),
        "Should fail to chunk file without read permissions"
    );
...
fn test_deeply_nested_directory_structure() {
    ...
    assert!(
        files[0].path.ends_with("deep.rs"),
