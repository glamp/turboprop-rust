...
async fn test_model_manager_caching() -> Result<()> {
    ...
    let expected_path = temp_dir.path().join("sentence-transformers_all-MiniLM-L6-v2");
...
async fn test_model_switching() -> Result<()> {
    ...
    for model_info in models.iter().take(2) {
        ...
        match generator.embed_batch(&test_texts) {
            ...
            Err(e) => {
                ...
                eprintln!("Could not generate embeddings for {}: {}", model_info.name, e);
            }
        }
    }
    ...
    if embeddings_by_model.len() >= 2 {
        ...
        assert!(emb1.len() > 0, "First model should produce non-empty embeddings");
        assert!(emb2.len() > 0, "Second model should produce non-empty embeddings");
...
async fn test_embedding_options() -> Result<()> {
    ...
    for options in options_variants {
        ...
        assert!(result.is_ok(), "Embedding generation should succeed with options: {:?}", options);
...
fn test_model_validation() -> Result<()> {
    ...
    for model in &models {
        assert!(model.validate().is_ok(), "Model {} should pass validation", model.name);
        ...
        match model.model_type {
            ...
            turboprop::types::ModelType::GGUF => {
                ...
                assert!(model.download_url.is_some(), "GGUF models should have download URL");
            },
    ...
    let has_sentence_transformer = models.iter().any(|m| matches!(m.model_type, turboprop::types::ModelType::SentenceTransformer));
    let has_gguf = models.iter().any(|m| matches!(m.model_type, turboprop::types::ModelType::GGUF));
    let has_huggingface = models.iter().any(|m| matches!(m.model_type, turboprop::types::ModelType::HuggingFace));
    
    assert!(has_sentence_transformer, "Should have at least one SentenceTransformer model");
    assert!(has_gguf, "Should have at least one GGUF model");
    assert!(has_huggingface, "Should have at least one HuggingFace model");
...
async fn test_batch_processing() -> Result<()> {
    ...
    for batch_size in batch_sizes {
        ...
        let result = generator.embed_batch(&test_texts);
        assert!(result.is_ok(), "Batch processing should succeed with batch size {}", batch_size);
        ...
        for (i, embedding) in embeddings.iter().enumerate() {
            assert_eq!(embedding.len(), 384, "Embedding {} should have correct dimensions", i);
        }
    }
...
fn test_model_specific_features() {
    ...
    if let Some(gguf_model) = models.iter().find(|m| matches!(m.model_type, turboprop::types::ModelType::GGUF)) {
        assert!(gguf_model.name.as_str().contains("gguf"), "GGUF model name should contain 'gguf'");
        assert!(gguf_model.description.to_lowercase().contains("code") || 
    ...
    if let Some(qwen_model) = models.iter().find(|m| m.name.as_str().contains("Qwen3")) {
        assert_eq!(qwen_model.model_type, turboprop::types::ModelType::HuggingFace);
        assert_eq!(qwen_model.backend, turboprop::types::ModelBackend::Custom);
        assert!(qwen_model.dimensions >= 1024, "Qwen3 should have high-dimensional embeddings");
    }
    
    // Test SentenceTransformer model features
    if let Some(st_model) = models.iter().find(|m| matches!(m.model_type, turboprop::types::ModelType::SentenceTransformer)) {
...
fn test_cosine_similarity() {
    ...
    assert!((cosine_similarity(&vec1, &vec2) - 1.0).abs() < 1e-6, "Identical vectors should have similarity 1.0");
    assert!((cosine_similarity(&vec1, &vec3) - 0.0).abs() < 1e-6, "Orthogonal vectors should have similarity 0.0");
}
...
async fn test_embedding_similarity_patterns() -> Result<()> {
    ...
    assert!(similar_similarity >= -1.0 && similar_similarity <= 1.0, "Similarity should be in [-1, 1] range");
    assert!(different_similarity >= -1.0 && different_similarity <= 1.0, "Similarity should be in [-1, 1] range");
